...
CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizar_tour`(
    IN p_id_tour INT,
    IN p_nuevo_nombre_tour VARCHAR(45),
...
BEGIN
    UPDATE tour
    SET 
        nombre_tour = p_nuevo_nombre_tour,
        valor = p_nuevo_valor,
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `consultar_tour`(in valor decimal(10,2))
BEGIN
select * from vista_tour where id_tour like concat('%',valor,'%') || nombre_tour like concat('%',valor,'%') || valor like concat('%',valor,'%') || fecha_de_inicio like concat('%',valor,'%') || fecha_final like concat('%',valor,'%') || descripción like concat('%',valor,'%') || diponibilidad like concat('%',valor,'%');
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_tour`(in des int, in nom varchar(45))
BEGIN
insert into tour (id_destino,nombre_tour, valor, fecha_de_inicio, fecha_final, descripcion, disponibilidad) values (des,nom_tour,val,fecha_ini,fecha_fin,descrip,dispo);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2024-07-10  9:09:07
